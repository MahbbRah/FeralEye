...
class AppConfig:
    ...
    RETENTION_LOCAL_DAYS: int = _get_int("RETENTION_LOCAL_DAYS", 10)
    # Google Drive evidence (per-event subfolders) older than this many days are deleted automatically.
    GDRIVE_RETENTION_DAYS: int = _get_int("GDRIVE_RETENTION_DAYS", 60)
    # How often (in seconds) the retention cleanup routine runs.
    RETENTION_CHECK_INTERVAL_SEC: float = _get_float("RETENTION_CHECK_INTERVAL_SEC", 3600.0)
